...
func _physics_process(_delta):
	if is_dashing:
		...
		var animation = velocity.normalized()
		if animation != Vector2.ZERO and !Gamemanager.player_robber:
			if abs(animation.x) > abs(animation.y):
				player.play("run")
				...
				if animation.y > 0:
					...
					player.play("up")
		elif !Gamemanager.player_robber:
			player.play("idle")
		
		if animation != Vector2.ZERO and Gamemanager.player_robber:
			if abs(animation.x) > abs(animation.y):
				player.play("robber_run")
				...
				if animation.y > 0:
					player.play("robber_down")
				else:
					player.play("robber_up")
		elif Gamemanager.player_robber:
			player.play("robber_idle")
...
func start_dash(dir):
	...
	if !Gamemanager.player_robber:
		...
		if abs(dir.x) > abs(dir.y):
			player.play("robber_flip_sideways")
			player.flip_h = dir.x <= 0
		else:
			if dir.y > 0:
				player.play("robber_flip_down")
			else:
				player.play("robber_flip_up")
